(function ($) {
    ...
    */
    ...
    function splitSentence(sentence, blockLength) {
        const blocks = [];
        var split_per_sentences = sentence.split('.');
        split_per_sentences.forEach(function (split_per_sentence) {
            blocks.push(split_per_sentence);
        });
    ...
    function speak(element) {
        try {
            ...
            if ('speechSynthesis' in window) {
                ...
                const desiredLanguage = 'id-ID'; // Desired language (e.g., English, United States)

                speech.rate = 0.98; // Speech rate (adjust as needed)
                ...
                let filtered_voices = voices.filter(voice => voice.lang.includes(desiredLanguage));
                var name_options = [
                    'Google Bahasa Indonesia',
                    'Microsoft Irina - Indonesian (Indonesia)',
                    'Apple Tessa Compact - Indonesian',
                    'Apple Tessa - Indonesian',
                ];

                // get all voices based on name options
                var selectedVoice = filtered_voices.find(function (voice) {
                    return name_options.includes(voice.name);
                });

                if (filtered_voices.length == 0) {
                    filtered_voices = voices;
                }

                // Set the selected voice (or the first available voice if not found)
                speech.voice = selectedVoice ?? filtered_voices[0];

                currentlySpeaking = false;
                var voice_name = speech.voice.name ?? '';
        ...
        init: function () {
            ...
            $('#fb5-page-number').keydown(function (e) {
                if (e.keyCode == 13) {
                    ...
                    halamanYangDiakses = parseInt($('#fb5-page-number').val(), 10);
                    if ((halamanYangDiakses % 2) == 0) {
                        halamanYangDiakses += 1;
                    }
                }
            });
